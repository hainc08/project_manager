...
router.post('/', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    ...
    const id = generateId();
    await db.prepare('INSERT INTO project_items (id, name, description) VALUES (?, ?, ?)')
...
router.put('/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    ...
    const item = await db.prepare('SELECT id FROM project_items WHERE id = ?').get(id);
    if (!item) {
...
router.delete('/:id', authenticate, authorize('ADMIN'), async (req, res) => {
  try {
    ...
    const usage = await db.prepare('SELECT project_id FROM project_item_mapping WHERE item_id = ? LIMIT 1').get(id);
    if (usage) {
    ...
    const taskUsage = await db.prepare('SELECT id FROM tasks WHERE project_item_id = ? LIMIT 1').get(id);
    if (taskUsage) {
      return res.status(400).json({ error: 'Không thể xóa hạng mục này vì đang có công việc liên kết' });
    }

    await db.prepare('DELETE FROM project_items WHERE id = ?').run(id);
